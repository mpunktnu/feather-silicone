...
//Här är de olika fallen som styrs från kontrollpanelen i appen    
  ...
  if (animationState == 2){ // Knapp "2" i control pad - tänd hela listen en "pixel" i taget med vald färg, släck därefter på samma sätt
    ...
    pixel.show(); // This sends the updated pixel color to the hardware.
    colorWipe(pixel.Color(0, 0, 0), 40);
    pixel.show();
  }
  if (animationState == 3){ // Knapp "3" i control pad - svag regnbåge jämnt distribuerad över hela
    for(uint16_t i=0; i<pixel.numPixels(); i++) { //clear all pixels before displaying new animation
          pixel.setPixelColor(i, pixel.Color(0,0,0));
        }
  ...
  if (animationState == 4){ // Knapp "4" i control pad - stark regnbåge jämnt distribuerad över hela
    for(uint16_t i=0; i<pixel.numPixels(); i++) { //clear all pixels before displaying new animation
          pixel.setPixelColor(i, pixel.Color(0,0,0));
        }
  ...
  if (animationState == 5){ // uppåtpil - stark regnbåge, en färg i taget över hela listen
    for(uint16_t i=0; i<pixel.numPixels(); i++) { //clear all pixels before displaying new animation
          pixel.setPixelColor(i, pixel.Color(0,0,0));
        }
    pixel.setBrightness(255);
  ...
  if (animationState == 6){ // nedåtpil - svag regnbåge, en färg i taget över hela listen
    for(uint16_t i=0; i<pixel.numPixels(); i++) { //clear all pixels before displaying new animation
          pixel.setPixelColor(i, pixel.Color(0,0,0));
        }
    pixel.setBrightness(80);
...
//Nedan följer diverse hjälpfunktioner
